    ...
    GoogleDocsContentService googleDocsContentService)
{
    ...
    private async Task DisplayCourseWorkInformationBatched()
    {
        ...
        // Collect display information for all courses first
        var allDisplayData = new List<(
            string courseId,
        ...
        foreach (var (courseId, workItems) in validWorkByCourse)
        {
            ...
            allDisplayData.Add((courseId, workItems, allSubmissions));
        }

        // Now display all courses and collect display information
        var displayResult = DisplayAllCourses(allDisplayData);

        // Only ask for user input if there are courses to process
        if (displayResult.courseNumber > 0)
        {
            await ProcessSelectedCourse(displayResult.coursesList, displayResult.displayedCourseIndices);
        }
    }
